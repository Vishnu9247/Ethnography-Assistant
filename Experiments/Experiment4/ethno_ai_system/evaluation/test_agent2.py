"""Run Agent 2 on one row or all rows."""
...
from evaluation.metrics import score_agent2
...
def evaluate_row(row_number: int) -> Dict[str, float]:
    """Run Agent 2 and compute metrics for one row."""
    init_store()
    persona_data = get_persona_by_row(row_number)
    persona_agent = PersonaAgent(persona_data)
    session_id = f"agent2_row_{row_number}"

    intake_result = run_agent1(persona_agent, persona_data, session_id=session_id)
    result = run_agent2(persona_agent, persona_data, intake_result, session_id=session_id)
    scores = score_agent2(result, persona_data)
...
def save_results(rows: List[Dict[str, float]]) -> Path:
    """Save Agent 2 metrics as CSV."""
    ensure_directories()
    output_path = RESULTS_DIR / "agent2_scores.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Evaluate Agent 2.")
...
def main() -> None:
    ...
    print(f"Saved Agent 2 results to {output_path}")
